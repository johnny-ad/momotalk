...
async def get_last_user_message_time(character: str) -> str | None:
    """Get the timestamp of the most recent user message for a character."""
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT timestamp FROM messages WHERE character = ? AND role = 'user' ORDER BY timestamp DESC LIMIT 1",
            (character,),
        )
