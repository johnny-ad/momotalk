...
# Track whether tables have been initialized this session
_tables_initialized = False


async def init_group_tables():
    """Create group chat tables. Only runs once per session."""
...
async def claim_group_message_response(chat_id: int, message_id: int, claimed_by: str = "") -> bool:
    """Atomically claim a message so only one bot responds to it."""
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "INSERT OR IGNORE INTO group_message_responses (chat_id, message_id, claimed_by) VALUES (?, ?, ?)",
            (chat_id, message_id, claimed_by),
        )
        await db.commit()
...
CHATTINESS = {
    ...
    "shiroko": 0.15,   # Few words, doesn't type much
    "reisa": 0.25,     # Speaks about justice and vigilante stuff
    "ui": 0.15,        # Reclusive, barely types
