...
class CharacterBot:
    ...
    # Character name aliases for group chat mention detection
    ...
    def _is_mentioned_in(self, text: str) -> bool:
        """Check if this character is mentioned by name or alias in the text."""
        lower = text.lower()
        if self.CHARACTER_NAME.lower() in lower:
            return True
    ...
    async def _handle_group_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Per-bot cooldown — don't let the same bot respond too frequently
        ...
        # ── CLAIM FIRST: prevent multiple bots from responding ──
        # Add a small random jitter so all bots don't race at the exact same instant
        await asyncio.sleep(random.uniform(0.1, 1.5))

        # Hard block: if ANY bot responded in the last 15 seconds, skip entirely
        group_seconds_since = await get_seconds_since_group_last_response(chat_id)
        if group_seconds_since is not None and group_seconds_since < 15:
            return

        # Calculate chattiness
        chattiness = CHATTINESS.get(self.CHARACTER_NAME.lower(), 0.3)
        if period == "school":
            chattiness *= 0.3

        # Dampen if a bot responded recently (15-45s ago)
        if group_seconds_since is not None and group_seconds_since < 45:
            chattiness *= 0.15

        # Boost if directly mentioned by name or alias
        is_mentioned = self._is_mentioned_in(msg.text)

        if not should_respond_to_group(self.CHARACTER_NAME, sender_name, msg.text, chattiness):
            # Even if the dice roll failed, respond if directly mentioned
            if not is_mentioned:
                return

        # Try to atomically claim this message
        if msg.message_id is not None:
            claimed = await claim_group_message_response(chat_id, msg.message_id, self.CHARACTER_NAME)
            if not claimed:
                return
...
"""
    ...
    async def handle_photo(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        response = await get_response(
            character=self.CHARACTER_NAME.lower(),
            system_prompt=self._get_timed_prompt(),
            user_messages=[caption or "Sensei sent you this image."],
    ...
    def run(self):
        ...
        async def post_init(app: Application):
            await init_db()
            from group_chat import init_group_tables
